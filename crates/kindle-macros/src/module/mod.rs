    ...
    let module_impl = match input.data {
        Data::Struct(ref data) => {
            ...
            if let Some(ref kind) = contains_kind {
                k = Some(quote! {
                    #kind : kindle::prelude::Kind,
                });
            }
            if let Some(ref device) = contains_device {
                d = Some(quote! {
                    #device : kindle::prelude::Device,
                });
            }
            let mut out = quote! {
                impl<
                    KindleShape: kindle::prelude::Shape,
                    #k
                    #d
                > kindle::prelude::Module<
                    kindle::prelude::Tensor<
                        KindleShape,
            };
            let mut tmp = quote! {};
...
fn gen_body(
    ...
    quote! {{
        type Output = kindle::prelude::Tensor<
            #tensor_shape,
            #ty
            #k
            #d
        ...
        fn forward(&self, xs: &kindle::prelude::Tensor<KindleShape, #ty #k #d #device>) -> Self::Output {
            #first
            #(#forward)*
            xs
        }
...
fn gen_tensor_shape(start: usize, layer_types: &VecDeque<LayerType>) -> proc_macro2::TokenStream {
    if start == layer_types.len() {
        ...
        match layer_types[start].ty {
            Layers::Normal => quote! {
                <#middle as kindle::prelude::Forward<#(#dims),*>>::ForwardShape
            },
            Layers::Swiglu => quote! {
...
fn gen_where_clause(start: usize, layer_types: &VecDeque<LayerType>) -> proc_macro2::TokenStream {
    if start == layer_types.len() {
        ...
        match layer_types[start].ty {
            Layers::Normal => quote! {
                #middle: kindle::prelude::Forward<#(#dims),*>,
            },
            Layers::Swiglu => quote! {
